...
from utils import avg, get_response_json, logger, std
...
parser.add_argument(
    "--character", type=str, default="haruhi", help="character name or code"
)
...
# from characters import alias2character, character_info, character_labels
...
def load_questionnaire(questionnaire_name):
    q_path = os.path.join("evaluation", "questionnaires", f"{questionnaire_name}.json")
    ...
    if questionnaire_name not in dims_dict:
        dims_dict[questionnaire_name] = [
            _["cat_name"] for _ in questionnaire["categories"]
        ]
...
def subsample_questionnaire(questionnaire, n=20):
    ...
    def subsample(questions, key, n):
        ...
        for key_value in key_values:
            ...
            num_samples = (
                base_per_key + 1
                if key_value in keys_w_additional_question
                else base_per_key
            )
    ...
    if "sub_dimension" in questionnaire[0].keys():  # bigfive, old version
        ...
        for dim, dim_questions in dimension_questions.items():
            new_questionnaire += subsample(
                dim_questions, "sub_dimension", n // len(dimension_questions.keys())
            )
...
def build_character_agent(character_code, agent_type, agent_llm, questionnaire_name):
    ...
    if agent_type_args[0] == "ChatHaruhi":
        character_agent = ChatHaruhi(
            role_name=character_info[character_code]["agent"]["ChatHaruhi"],
            llm=agent_llm,
            questionnaire_name=questionnaire_name,
        )
    elif agent_type_args[0] == "RoleLLM":
        character_agent = ChatHaruhi(
            role_from_hf=f'silk-road/ChatHaruhi-from-RoleLLM/{character_info[character_code]["agent"]["RoleLLM"]}',
            llm=agent_llm,
            embedding="bge_en",
            questionnaire_name=questionnaire_name,
        )
        character_agent.role_name = (
            "RoleLLM/" + character_info[character_code]["agent"]["RoleLLM"]
        )
...
def interview(
    ...
    for question in tqdm(questionnaire):
        ...
        if query_style == "interview":
            ...
            q = (
                questionnaire_prompts["rpa_choose_prefix"][language].replace(
                    "<statement>", question[f"origin_{language}"]
                )
                + " "
                + questionnaire_prompts["rpa_choice_instruction"][language]
            )
            ...
            if query_style == "choosecot":
                if language == "en":
                    q = q.replace(
                        ...
                        "Please think step by step. Start by sharing your thoughts, then proceed to present the number.",
                    )
                else:
                    q = q.replace(
                        "请你只回答这一个数字，不要说其他内容。", "请给出你的理由。"
                    )
...
def assess(
    ...
    dims = dims_dict.get(
        questionnaire_name, sorted(list(set([q["dimension"] for q in questionnaire])))
    )
    ...
    character_aliases = sorted(
        set(character_aliases), key=lambda x: len(x), reverse=True
    )
    ...
    for r in questionnaire_results:
        ...
        if not (agent_llm.startswith("gpt")):
            from utils import (contain_repeation, is_multilanguage,
                               is_multiround, not_into_character, truncate)

            # import pdb; pdb.set_trace()

            if is_multilanguage(question, response):
                error_counts["is_multilanguage"] = (
                    error_counts.get("is_multilanguage", 0) + 1
                )
                # print(f'{response}\nis_multilanguage')
                # import pdb; pdb.set_trace()
            if not_into_character(response, experimenter):
                error_counts["not_into_character"] = (
                    error_counts.get("not_into_character", 0) + 1
                )
                # print(f'{response}\nnot_into_character')
                # import pdb; pdb.set_trace()
            if contain_repeation(response):
                error_counts["contain_repeation"] = (
                    error_counts.get("contain_repeation", 0) + 1
                )
                # print(f'{response}\ncontain_repeation')
                # import pdb; pdb.set_trace()
        ...
        if colon_idx == -1 and not any(
            [response.startswith(a) for a in character_aliases]
        ):
            r["response_open"] = (
                character_name + ": 「" + r["response_open"].strip("「」 :") + "」"
            )

    global previous_file_path
    previous_file_path_cp = previous_file_path.replace(
        "../results/assessment/", "../results/assessment_cp/"
    )
    if os.path.exists(previous_file_path_cp):
        previous_file_path = previous_file_path_cp

    if False and os.path.exists(previous_file_path):
        ...
        options = [
            ...
            for i in range(
                ...
                questionnaire_metadata["range"][1] + 1,
            )
        ]
        choices = {}
        ...
        if "adjoption" in eval_args:
            ...
            need_convert_list = [
                need_convert[i : i + 60] for i in range(0, len(need_convert), 60)
            ]
        ...
        for need_convert in need_convert_list:
            ...
            for r in need_convert:
                r_ = r.copy()
                r_["question"] = r_["question"].replace(
                    questionnaire_metadata["prompts"]["rpa_choice_instruction"][
                        language
                    ],
                    "",
                )
            ...
            if "adjoption" in eval_args:
                ...
                sys_prompt = (
                    questionnaire_metadata["prompts"]["convert_to_choice"][
                        "en"
                    ].replace(
                        "agrees with the statement",
                        f"displays a highly {need_convert_dim} personality",
                    )
                    + "\n"
                ...
                sys_prompt = (
                    questionnaire_metadata["prompts"]["convert_to_choice"]["en"]
                    + "\n"
                    + questionnaire_metadata["prompts"]["llm_choice_instruction"]["en"]
            ...
            if (
                evaluator_llm == "gpt-3.5"
                and num_tokens_from_messages(
                    [
                        {
                            "role": "user",
                            "content": json.dumps(
                                need_convert, indent=4, ensure_ascii=False
                            ),
                        }
                    ],
                    "gpt-3.5-turbo-1106",
                ...
                need_convert_list = [
                    {str(j + 1): need_convert[str(j + 1)] for j in range(i, i + 30)}
                    for i in range(0, len(need_convert), 30)
                ]
            else:
            ...
            for need_convert in need_convert_list:
                ...
                if evaluator_llm.startswith("gpt"):
                    ...
                    converted_choices = get_response_json(
                        [string2json_ensure_keys],
                    ...
                    converted_choices = get_response_json(
                        [string2json_ensure_choice_format, string2json_ensure_keys],
                        sys_prompt=sys_prompt,
                        inputs=user_input,
                        model=evaluator_llm,
                    )
        ...
        for dim in tqdm(dims):
            ...
            dim_responses = [
                r
                for i, r in enumerate(questionnaire_results)
            ...
            for batch_responses in dim_responses_list:
                ...
                if questionnaire_name == "16Personalities":
                    ...
                    output_format_prompt = prompts["general"][
                        "two_score_output"
                    ].format(dim_cls1, dim_cls2)
                ...
                sys_prompt = sys_prompt.replace(
                    ...
                    "Other numbers in this range represent different degrees of 'Conscientiousness'. You must give a score, and you are not allowed to give answers like 'N/A' and 'not applicable'.",
                    1,
                )

                llm_response = get_response_json(
                    sys_prompt=sys_prompt, inputs=user_input, model=evaluator_llm
                )
    ...
    if questionnaire_name == "16Personalities" and not "assess" in eval_args:
        ...
        answers = [
            4 - int(id2answer[str(i)]) if id2answer[str(i)] != "x" else 0
            for i in range(1, 61)
        ]
...
def personality_assessment(
    ...
    if questionnaire_name in scale_list:
        ...
        print(
            f"Questionnaire {questionnaire_name} not found. Here are the items: {scale_list}"
        )
        raise NotImplementedError
    ...
    if rerun or not os.path.exists(final_save_path):
        ...
        if eval_method == "direct":
            if not character_agent:
                ...
                character_agent = build_character_agent(
                    character, agent_type, agent_llm, "16personalities"
                )
            ...
            if repeat_times < 1:
                ...
                subsample_questionnaire_path = os.path.join(
                    subsample_questionnaire_folder_path, f"{questionnaire_name}.json"
                )

                if not os.path.exists(subsample_questionnaire_path):
                    questionnaire = subsample_questionnaire(
                        questionnaire, n=math.ceil(len(questionnaire) * repeat_times)
                    )
                    with open(subsample_questionnaire_path, "w") as f:
                        json.dump(questionnaire, f, indent=4, ensure_ascii=False)

                    logger.info(
                        f"Subsample questionnaire and save into {subsample_questionnaire_path}"
                    )
                else:
                    logger.info(
                        f"Load subsampled questionnaire from {subsample_questionnaire_path}"
                    )
            ...
            interview_folder_path = f"results/generation/dataset={dataset}_agent-llm={agent_llm}_retrieval-method={retrieval_method}"
            for nth_test in range(max(repeat_times, 1)):
                ...
                interview_save_path = os.path.join(
                    interview_folder_path, interview_save_path
                )
                if not os.path.exists(interview_save_path):
                    ...
                    with open(interview_save_path, "w") as f:
                        json.dump(
                            questionnaire_results, f, indent=4, ensure_ascii=False
                        )
                    logger.info(
                        f"Interview finished... save into {interview_save_path}"
                    )
                else:
                    logger.info(
                        f"Interview done before. load directly from {interview_save_path}"
                    )
                ...
                assessment_save_path = os.path.join(
                    assessment_folder_path, assessment_save_path
                )
                ...
                if rerun or not os.path.exists(assessment_save_path):
                    ...
                    logger.info(
                        f"Assess done before. load directly from {assessment_save_path}"
                    )
        ...
        assessment_results = {"dims": {}, "analysis": {}, "code": ""}
            ...
            if "assess" not in eval_method:
                for a_results in multitime_assess_results:
                    for item in a_results[dim]["details"]:
                        ...
                        item_id = item["id"][0]
                        multitime_item_results[item_id] = multitime_item_results.get(
                            item_id, []
                        ) + [item["score"]]

        score_span = (
            questionnaire_metadata["range"][1] - questionnaire_metadata["range"][0]
        )
        ...
        if "assess" not in eval_method:
            assessment_results["analysis"]["item_consistency"] = (
                avg([std(scores) for item_id, scores in multitime_item_results.items()])
                / score_span
            )

            assessment_results["analysis"]["dim_consistency"] = (
                avg([avg(assessment_results["dims"][dim]["intra_std"]) for dim in dims])
                / score_span2
            )

        if repeat_times > 1:
            assessment_results["analysis"]["robustness"] = (
                avg([assessment_results["dims"][dim]["inter_std"] for dim in dims])
                / score_span2
            )
        ...
        if questionnaire_name in ["BFI", "16Personalities"]:
            thresh = (
                questionnaire_metadata["range"][0] + questionnaire_metadata["range"][1]
            ) / 2
            ...
            for label_setting in label_settings:
                labels = character_labels[label_setting][character_name][
                    questionnaire_name
                ]
    ...
    for nth_test in range(repeat_times):
        ...
        for rpa in preds.keys():
            ...
            for dim in label.keys():
                ...
                sum_mse_each_dim[a][dim] += (
                    (pred_score - label_score) / range_span
                ) ** 2
        ...
        for count in [
            sum_mse_each_dim,
            sum_mae_each_dim,
            correct_single_each_dim,
            count_single_each_dim,
        ]:
        ...
        for a in agent_types + ["all"]:
            ...
            for dim in dims + ["all"]:
                single_acc[dim] = (
                    correct_single_each_dim[a][dim] / count_single_each_dim[a][dim]
                )
                single_mse[dim] = (
                    sum_mse_each_dim[a][dim] / count_single_each_dim[a][dim]
                )
                single_mae[dim] = (
                    sum_mae_each_dim[a][dim] / count_single_each_dim[a][dim]
                )
    ...
    for a in agent_types + ["all"]:
        for metric in metrics[a].keys():
            if isinstance(metrics[a][metric], dict):
                for dim in dims + ["all"]:
                    metrics[a][metric][dim] = avg(
                        [m[a][metric][dim] for m in multitime_metrics]
                    )
            else:
